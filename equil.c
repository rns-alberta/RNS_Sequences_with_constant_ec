   ...
   for(s=1;s<=SDIV;s++) {
    ...
    for(m=1;m<=MDIV-2;m+=2) {
     ...
     D_vp[s] += (1.0/(3.0*(MDIV-1)))*( exp(2.0*alpha[s][m]+(gama[s][m]
             ...
             Rv= s_gp[s]*r_e*sqrt(KAPPA)/((1.-s_gp[s])*100000.);
    }    
...
/* Rescale r_is and compute lambda */
   ...
   if(i_check==3) {
    ...
    //printf( "%7g %7g\n", 
     //(*r_final)*sqrt(KAPPA)/100000., (*m_final)*sqrt(KAPPA)*C*C/(G*MSUN));
}
...
double   sum_rho=0.0,         /* intermediate sum in eqn for rho */
  ...
  while(dif> accuracy || n_of_it<2) { 
      ...
      */        
      ...
      /*FILE *out; 
      out = fopen("par9.txt","w");*/
      /*FILE *out1;
      out1 = fopen("pot.txt","w");
      
      if(r_ratio==1.00){ 
      fclose(out1);
      FILE *out1;
       out1 = fopen("pot1.txt","w");
      }
      if(r_ratio>0.899&&r_ratio<0.901){ 
      fclose(out1);
      FILE *out1;
       out1 = fopen("pot2.txt","w");
      }
      if(r_ratio>0.799&&r_ratio<0.801){ 
      fclose(out1);
      FILE *out1;
       out1 = fopen("pot3.txt","w");
      }
      if(r_ratio>0.699&&r_ratio<0.701){ 
      fclose(out1);
      FILE *out1;
       out1 = fopen("pot4.txt","w");
      }
      if(r_ratio>0.599&&r_ratio<0.601){ 
      fclose(out1);
      FILE *out1;
       out1 = fopen("pot5.txt","w");
      }*/
            ...
            if((enthalpy[s][m]<=enthalpy_min) || (sgp>s_e)) {
                  ...
                  energy[s][m]=0.0; 
                     ...
                     energy[s][m]=e_at_p(pressure[s][m], log_e_tab, 
                       ...
                       //akonst
                  if(s_gp[s]*r_e*sqrt(KAPPA)/((1.-s_gp[s])*100000.)>=0){
                  //      fprintf(out,"%5.4e %5.4e %5.4e %5.4e %5.4e %5.4e %5.4e %5.4e\n",s_gp[s],s_gp[s]*r_e*sqrt(KAPPA)/((1.-s_gp[s])*100000.),mu[m],pressure[s][m]*(C*C*C*C)/(KAPPA*G),energy[s][m]*(C*C)/(KAPPA*G),enthalpy[s][m]*C*C,(energy[s][m]*(C*C)/(KAPPA*G)+pressure[s][m]*(C*C)/(KAPPA*G))*exp(-enthalpy[s][m]),velocity_sq[s][m]);
                   //printf("%5.4e \n",s_gp[s]*r_e*sqrt(KAPPA)/((1.-s_gp[s])*100000.));
                      // if(velocity_sq[s][m]<50. && mu[m]<2.0 ){ 
                           /*fprintf(out1,"%5.4e %5.4e %5.4e %5.4e %5.4e %5.4e %5.4e %5.4e %5.4e %5.4e %5.4e %5.4e\n",s_gp[s],s_gp[s]*r_e*sqrt(KAPPA)/((1.-s_gp[s])*100000.),mu[m],rho[s][m],gama[s][m],alpha[s][m],omega[s][m],pressure[s][m],energy[s][m],enthalpy[s][m],(energy[s][m]+pressure[s][m])*exp(-enthalpy[s][m]),velocity_sq[s][m]);*/
                        // }
                       }
